...
- [x] R2. Свести изменение response-card к одному per-user coordinator вместо
  ...
  response-card.
- [ ] R3. Разделить крупные orchestrator-файлы по самостоятельным обязанностям
...
- [ ] R4. После R1-R3 провести аудит компоновки Go packages по официальным
  рекомендациям Go. Сохранить один `go.mod`, `cmd/bria` как composition root и
  production-код в `internal`; проверить связность ответственности, fan-out,
  consumer-owned узкие interfaces и отсутствие циклов/пакетов вида
  `util`/`common`/`api`/`types`/`interfaces`. Отдельно устранить зависимость
  нейтрального application-слоя от `telegramui` и проверить границы
  `telegramapp`, не создавая формальных micro-packages.

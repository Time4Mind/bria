...
- [x] R4. После R1-R3 провести аудит компоновки Go packages по официальным
  ...
  - [x] R4.4. Зафиксировать проверяемое направление зависимостей и отсутствие
    циклов и пакетов вида `util`/`common`/`api`/`types`/`interfaces`.
  - [x] R4.5. Подтвердить итоговую компоновку: один `go.mod`, `cmd/bria` как
...
- [x] S1. Гарантированно убирать Bria-owned tmux runtime после терминально
  ...
  логировать.
- [x] S2. Сделать shutdown runtime транзакционным: прекратить admission,
...
- [x] S3. Ограничить per-session FIFO и ввести явный backpressure для потока
  runtime-команд при зависшем или ещё не attached provider. Пользовательские

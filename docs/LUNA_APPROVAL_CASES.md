...
Decision claim внутри flow относится к активному диалогу provider/generation,
а не его меняющемуся при resize fingerprint. Unknown result не вызывает повтор
при collapsed → full. Следующий epoch требует наблюдения непустого
неинтерактивного экрана; пустая перерисовка не сбрасывает claim. У CLI нет
структурированного request ID в capture: если два одинаковых диалога сменились
без наблюдаемого промежутка, автоматика может удержать второй. Внутрипроцессный
claim не является durable runtime-журналом через перезапуск контроллера;
durable O_EXCL claim предусмотрен отдельно у diagnostic helper.

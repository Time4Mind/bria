...
| A41.7 | Полный выпуск и postflight | full gate, CI, hashes, service health | in progress | Финальный post-review `VERSION=20260910-persistent-preprocess-satellites make check-full` PASS; независимый review APPROVE и повторный live Luna E2E PASS за 10,89 с. Открыты commit/push, exact-SHA CI и live service postflight. |
| A41.8 | Повторяющийся recovery failure без падения процесса | bounded RCA и regression probe | verified locally | Семь отдельных `session.recovery_failed` для ab396f5b... в 19:26-19:58 MSK после hotfix; интервалы 0/1/2/4/8/16 минут, PID 88039 и runs=1. Причина - неизменный `history unverifiable` barrier ошибочно считался временным retry. Новый payload-free stable evidence barrier не повторяет попытку/лог без изменения evidence или binding identity; targeted race и 20x regression PASS. Live postflight выпуска ещё открыт. |

...
> карточки A. Issue #1 исключён по его явному issue-only scope. Договор и
> coverage: [OPEN_ISSUES_3_4_TODO.md](OPEN_ISSUES_3_4_TODO.md).

> Текущий запрос A56 - исправить GitHub issue #2: голосовое, принятое в
> сессии A, после переключения пользователя на B должно остаться привязанным к
> A. Причина подтверждена RED-тестом: после распознавания терялся уже захваченный
> session ID и заново читалась mutable active session. Договор и evidence:
> [VOICE_SESSION_BINDING_TODO.md](VOICE_SESSION_BINDING_TODO.md).

...
В «Настройки → Кнопки сессии» находится только переключатель «Скрин»: он
включает или выключает PNG текущего native terminal snapshot в карточке.

В «Настройки → Содержимое карточки» поле «Качество скрина» циклически выбирает
один из трёх глобальных профилей:

| Значение в settings | Отображение | Разрешение | Палитра |
|---|---|---:|---:|
| `full_8` | `100%, 8 цветов` | 100% | не более 8 используемых цветов |
| `compact_8` | `75%, 8 цветов` | 75% по каждой оси | не более 8 используемых цветов |
| `current` | `100%, полная палитра` | 100% | прежнее полноцветное кодирование |

Дефолт `screen_image_profile` - `full_8`, в том числе когда поле отсутствует в
settings прежней версии. Профиль меняет только пиксельное представление PNG:
количество захваченного terminal text по-прежнему отдельно задаётся
`screen_capture_limit_kib` через поле «Размер захвата скрина» в том же разделе.
Это верхняя граница хвоста terminal text в KiB, а не разрешение картинки.
Native adapter держит безопасный максимум `86 KiB`, поэтому выбранный лимит
применяется renderer-ом к уже работающей сессии без её пересоздания. После
смены лимита или профиля прежний PNG и подтверждённый Telegram file ID не
переиспользуются.

### Скрытые каталоги

В «Настройки → Создание сессии» кнопка «Скрытые каталоги» переключает
«скрывать / показывать». По умолчанию дочерние директории с точкой в начале
имени не показываются; `project.v2` остаётся видимым. Выбор сохраняется в
`show_hidden_directories` файла `<state_path>.settings.json` (по умолчанию
`false`, в том числе при отсутствии поля в старом файле), общий для всех нод.
Фильтр применяется до пагинации и лимита списка. Он не запрещает доступ к
явно заданной папке, не меняет корни, родительский переход или существующие сессии.
После записи нового поля старые строгие версии кодека его не читают:
binary-only rollback требует отдельно подготовленного совместимого settings-файла.

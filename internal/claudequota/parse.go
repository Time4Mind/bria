// Package claudequota obtains Claude's read-only subscription usage view
// through an isolated, supervised CLI session and parses its bounded output.
package claudequota
...
func Parse(text string, nodeID domain.ComputerID, now time.Time) (telegramstatus.Snapshot, bool) {
	snapshot := telegramstatus.Snapshot{ComputerID: nodeID, Provider: domain.ProviderClaude, CollectedAt: now.UTC()}
	section, found := "", false
	...
	if !found {
		return telegramstatus.Snapshot{}, false
	}
	return snapshot, true
}

func snapshotWindow(snapshot *telegramstatus.Snapshot, section string) *telegramstatus.Window {
	switch section {
	case "session":
		if snapshot.FiveHour == nil {
			snapshot.FiveHour = &telegramstatus.Window{}
		}
		return snapshot.FiveHour
	case "week":
		if snapshot.Weekly == nil {
			snapshot.Weekly = &telegramstatus.Window{}
		}
...
func resetTime(value string, now time.Time) time.Time {
	clock, date := clockValue.FindStringSubmatch(value), monthDay.FindStringSubmatch(value)
	if date == nil {

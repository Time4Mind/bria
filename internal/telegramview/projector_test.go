...
func allowedNodeTokenAction(action telegramui.Action) bool {
	...
	case telegramui.ActionSelectNode, telegramui.ActionSelectArchiveNode,
		...
		telegramui.ActionNewSession, telegramui.ActionConfirmLeader,
...
func projectorFixture(t *testing.T) (*telegramview.Projector, *domain.State, *projectionTokens) {
	t.Helper()
	...
	projector, err := telegramview.New(projectionReader{state}, tokens)
	if err != nil {
		t.Fatal(err)
	}

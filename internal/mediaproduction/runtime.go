...
func Open(downloader mediaflow.Downloader, config Config) (*Runtime, error) {
	...
	if policy, ok := config.DocumentPolicy.(interface {
		WithDefaultAttacher(mediaflow.PhotoAttacher) mediaflow.DocumentPolicy
	}); ok && !nilPort(policy) {
		config.DocumentPolicy = policy.WithDefaultAttacher(photos)
	}

...
func TestHiddenDirectoriesActionIsGlobalAndRejectsSessionTargets(t *testing.T) {
	kind := telegramsemantic.SemanticSettingsHiddenDirectories
	if !telegramsemantic.IsGlobal(kind) {
		t.Fatal("hidden-directory setting is not global")
	}

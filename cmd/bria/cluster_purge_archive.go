...
func purgeClusterArchive(arguments []string) error {
	flags := flag.NewFlagSet("cluster purge-archive", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	configPath := flags.String("config", "", "current leader config path")
	sessionID := flags.String("session", "", "archived session ID")
	archiveID := flags.String("archive", "", "archive bundle ID; empty for a legacy unready record")
	...
	if flags.NArg() != 0 || *configPath == "" || *sessionID == "" || *revision == 0 {
		return errors.New("usage: bria cluster purge-archive --config PATH --session SESSION --revision N [--archive ID] --confirm NODE/SESSION")
	}
	...
	command, err := clusterstate.NewCommand(
		fmt.Sprintf("purge-archive-%s-%d", *sessionID, time.Now().UnixNano()),
		clusterstate.CommandPurgeSession, time.Now(), clusterstate.PurgeSession{
			Session: ref, ArchiveID: *archiveID, ExpectedRevision: *revision,
		},
	)
	if err != nil {
		return err
	...
	fmt.Printf("purged archived session %s\n", ref.Key())
	return nil
}

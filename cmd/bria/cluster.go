...
func runCluster(arguments []string) error {
	if len(arguments) == 0 {
		return errors.New("usage: bria cluster <init|join|contract|claim|set-peers|issue-node|cert-renew|recover-ca|set-owner|backup|restore|recover-archive|purge-archive|retire-node|relocate-node>")
	}
	switch arguments[0] {
	case "init":
	...
	case "recover-archive":
		return recoverClusterArchive(arguments[1:])
	case "purge-archive":
